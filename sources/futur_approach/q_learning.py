...
input_data = pd.read_csv('./input/btc_usd_1min.csv',
                   delimiter=',',
...
data = DataFrame(input_data["close"]).copy()


#==============================
data = data.join(input_data.ta.rsi(length=14*60))

macd_a = data["close"].rolling(window = 60).mean()
macd_b = data["close"].rolling(window = round(60*26/12)).mean()
data["macd"] = macd_a - macd_b
data = data.join(input_data.ta.cci(length=6*60))
...
model = tf.keras.models.load_model("./input/m2")
...
#%%
#=========================================
# ML Experience Replay
#=========================================

# actions: 3 => BUY, SELL, HOLD
# states: stock price, balance, coins
