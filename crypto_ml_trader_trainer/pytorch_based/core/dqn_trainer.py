...
class DQNTrainer:
    ...
    def optimize_model(self):
        ...
        non_final_next_states = [s for s in batch.next_state if s is not None] # list of tensors with 1 row
        if len(non_final_next_states) == 0:
            return

        non_final_next_states = torch.cat(non_final_next_states) # tensor 2d from the list of tensors
